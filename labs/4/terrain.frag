...
void main(void)
{
	...
	if (vertex.y == 0)
	{
		outColor = (vec4(ambient, 1.0) + vec4(diffuse, 1.0)) * vec4(0.2, 0.2, 1, 1) + vec4(specular, 1.0);
	}
	else
	{
		outColor = (vec4(ambient, 1.0) + vec4(diffuse, 1.0)) * texture(texUnit, texCoord) + vec4(specular, 1.0);
	}	
